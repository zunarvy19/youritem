...
const progressPct = computed(() => {
    ...
    return Math.min(100, Math.round((props.availableBudget / props.item.estimated_price) * 100));
});
</script>
...
<template>
    <article class="card p-5">
        <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
                <h3 class="truncate font-semibold text-neutral-900">{{ item.name }}</h3>
        ...
        <p class="mt-3 text-lg font-bold text-neutral-400 line-through decoration-rose-300/60">
            {{ formatIdr(item.estimated_price) }}
        </p>

        <div class="mt-3">
            <div class="flex items-center justify-between text-[11px] font-medium text-neutral-400">
