...
defineProps<{
    title: string;
    description?: string;
    icon?: 'heart' | 'bag' | 'clock';
}>();
</script>

<template>
    <div
        class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-neutral-300 bg-white px-6 py-16 text-center"
    >
        <span
            v-if="icon"
            class="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-400"
            aria-hidden="true"
        >
            <svg
